                        ...
                        <div className="px-5 md:px-6 py-4 md:py-5 space-y-5">
                            {editorialParagraphs.map((para, idx) => (
                                <div key={idx} className={idx > 0 ? "pt-4 border-t border-gray-100" : ""}>
                                    {para.english && (
                                        <p className="text-[14px] md:text-[15px] text-gray-800 leading-[1.7] mb-3">
                                            {renderBold(para.english)}
                                        </p>
                                    )}
                                    {para.hindi && (
                                        <p className="text-[14px] md:text-[15px] text-gray-600 leading-[1.7] border-l-2 border-amber-300 pl-3 italic">
                                ...
                                <div className="relative flex items-stretch">
                                    ...
                                    <div className="flex-1 bg-white px-4 md:px-5 pt-3 pb-1 flex items-start gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <h3 className="text-[14px] md:text-[16px] font-bold text-gray-900 leading-snug mt-0.5">{q.stem}</h3>
                                        </div>
                                    ...
                                    {q.explanation && (
                                        <div className="relative bg-gradient-to-br from-amber-50 to-white border border-amber-200/50 rounded-xl p-5 md:p-7 mt-4 shadow-sm">
                                            ...
                                            <div className="pl-4">
                                                ...
                                                <p className="text-[13px] md:text-[14px] text-amber-900 leading-relaxed">
                                                    {renderBold(q.explanation)}
                                                </p>
                                            </div>
                                        </div>
                                    )}
                    ...
                    <div className="border-t border-slate-200">
                        ...
                        <div className="max-w-[860px] mx-auto px-5 py-4">
                            <details className="bg-white rounded-lg border border-gray-100 shadow-sm overflow-hidden">
                                <summary className="bg-gradient-to-r from-[#0f172a] via-[#d97706] to-[#0f172a] px-5 md:px-6 py-3.5 cursor-pointer hover:opacity-90 transition-opacity text-[14px] font-bold text-white flex items-center gap-2 select-none">
                                    <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z" />
                                    </svg>
                                    <span>📖 Reading Passage &mdash; Daily News Vocab Quiz</span>
                                </summary>
                                <div className="px-5 md:px-6 pb-5 md:pb-6 border-t border-gray-100">
                                    <div className="space-y-4 mt-4">
                                        <p className="text-[13px] font-bold text-gray-700 italic">{title}</p>
                                        {editorialParagraphs.map((para, idx) => (
                                            <div key={idx} className="border-l-2 border-gray-200 pl-4">
                                                {para.english && (
                                                    <p className="text-[13px] text-gray-600 leading-relaxed">{para.english}</p>
                                                )}
                                                {para.hindi && (
                                                    <p className="text-[12px] text-gray-500 italic mt-1">{para.hindi}</p>
                                                )}
                                            </div>
                                        ))}
                                    </div>
                                    <div className="mt-4 pt-3 border-t border-gray-100">
                                        <p className="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-2">Questions Summary</p>
                                        <div className="flex flex-wrap gap-2">
                                            {sections.map((sec) => {
                                                const sectionQs = allQuestions.filter((q) => q.sectionNumber === sec.number);
                                                return (
                                                    <span key={sec.number} className={`inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold rounded-full ${SECTION_COLORS[sec.type] || "bg-gray-500"} text-white`}>
                                                        {sec.number}. {sec.typeName} ({sectionQs.length})
                                                    </span>
                                                );
                                            })}
                                        </div>
                        ...
                        <div className="flex flex-col bg-slate-50" style={{ minHeight: "calc(100vh - 96px)" }}>
                            ...
                            <div className="bg-slate-100 border-b border-slate-200 px-2 md:px-4 py-2 flex items-center overflow-x-auto whitespace-nowrap text-[10px] md:text-xs gap-1 shrink-0">
                                <span className="text-[#d97706] font-black uppercase mr-2 border-r border-slate-300 pr-2 tracking-wider text-[12px]">Daily News Vocab</span>
                                <span className="text-slate-500 font-bold uppercase mr-2 text-[11px]">{date}</span>
                                        ...
                                        <div className="flex-1 overflow-y-auto min-h-0 px-5 pb-5">
                                            ...
                                            {showExplanation && (
                                                <div className="bg-amber-50 border border-amber-200 rounded-lg p-5 mt-5">
                                                    ...
                                                    <p className="text-[13px] md:text-[14px] text-amber-900 leading-relaxed">
                                                        {renderBold(currentQuestion.explanation)}
                                                    </p>
                                                </div>
                                            )}
                                        </div>
