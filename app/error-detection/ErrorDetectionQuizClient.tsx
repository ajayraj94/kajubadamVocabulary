...
const markdownComponents = {
  ...
  th: ({ children, ...props }: React.ComponentPropsWithoutRef<"th">) => (
    <th
      className="border border-amber-300 bg-amber-100 px-4 py-2 font-bold text-amber-900 text-left text-[13px] md:text-[14px]"
      {...props}
    >
      {children}
    </th>
  ...
  td: ({ children, ...props }: React.ComponentPropsWithoutRef<"td">) => (
    <td
      className="border border-amber-200 px-4 py-2 text-gray-700 text-[13px] md:text-[14px]"
      {...props}
    >
      {children}
    </td>
  ...
  hr: (props: React.ComponentPropsWithoutRef<"hr">) => (
    <hr className="border-t border-amber-200 my-5" {...props} />
  ),
...
export default function ErrorDetectionQuizClient({
  questions,
  page,
  totalPages,
  totalQuestions,
  ...
  const { hasErrorDetection, isLoading: accessLoading } = usePurchaseAccess();
  const isLocked = !accessLoading && !hasErrorDetection && !isFreePage;
    ...
    return (
      <div className="min-h-screen bg-gray-50/40 font-sans flex items-center justify-center">
        <div className="text-center">
          <div className="w-6 h-6 border-4 border-[#8B0000] border-t-transparent rounded-full animate-spin mx-auto mb-3" />
  ...
  if (isLocked) {
    return (
      <div className="min-h-screen bg-[#0a0f1e] font-sans flex flex-col">
        <header className="bg-[#8B0000] text-white px-4 py-3 flex items-center gap-3 shadow-sm">
          <Link
            href="/"
            className="hover:bg-[#6B0000] p-1.5 rounded-full transition-colors"
          >
          ...
          <div>
            <h1 className="font-bold text-sm tracking-wide">SSC Error Detection</h1>
            <p className="text-xs text-red-100 font-semibold">716 PYQ &mdash; Premium Content</p>
          </div>
        </header>
        <div className="flex-1 flex items-center justify-center px-4 py-16">
          <div className="max-w-md w-full">
            ...
            <div className="text-center mb-8">
              <div className="inline-block text-[11px] font-black px-3 py-1 rounded-full mb-3 uppercase tracking-wider bg-red-500/10 border border-red-500/20 text-red-400">Premium Content &mdash; &#8377;110</div>
              <h2 className="text-white text-[24px] font-black mb-2 leading-tight">SSC Error Detection 716 PYQ</h2>
              <p className="text-gray-400 text-[14px] leading-relaxed">
                Master <strong className="text-white">716 Error Detection questions</strong> with detailed bilingual explanations. Ek baar purchase karo, lifetime access pao.
              </p>
            </div>
            <div className="bg-white/5 border border-white/10 rounded-xl p-5 mb-6">
              ...
              <ul className="space-y-2">
                {[
                  "716 SSC Error Detection PYQs (fully solved)",
            ...
            <div className="flex flex-col gap-3">
              <Link href="/pricing" className="w-full text-center bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white font-black text-[16px] py-4 rounded-xl transition-all duration-200 shadow-lg shadow-red-900/30 active:scale-[0.98]">
                &#128274; Unlock SSC Error Detection &mdash; &#8377;110
              </Link>
  ...
  return (
    <div className="min-h-screen bg-[#faf5ef] font-sans flex flex-col overflow-x-hidden">
      {/* TOP HEADER */}
      <header className="bg-[#8B0000] text-white px-4 py-3 flex justify-between items-center shadow-sm sticky top-0 z-30">
        <div className="flex items-center space-x-2">
          <Link href="/?tab=error-detection" className="hover:bg-[#6B0000] p-1.5 rounded-full transition-colors">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7" />
            </svg>
          </Link>
          ...
          <div>
            <h1 className="font-bold text-sm tracking-wide">SSC Error Detection</h1>
            <p className="text-xs text-red-200 font-semibold truncate max-w-[200px] md:max-w-[400px]">
        ...
        <div className="flex items-center gap-2">
          {!quizStarted && (
            <button
              ...
              className="flex items-center gap-1.5 bg-white text-[#8B0000] font-black text-[11px] uppercase tracking-wider px-4 py-2 rounded-lg shadow-md hover:bg-red-50 transition-all hover:scale-105 active:scale-95"
            >
          ...
          {quizStarted && Object.keys(answers).length > 0 && (
            <div className="flex items-center space-x-4 text-sm">
              <div className="text-right leading-tight">
                ...
                <div className="text-[10px] text-red-200">+{correctCount} / -{(wrongCount * 0.25).toFixed(2)}</div>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="space-y-8 md:space-y-10">
        {questions.map((q, qIdx) => (
          ...
          >
                ...
                <div className="relative flex items-stretch">
                  ...
                  <div className="flex-1 bg-white px-4 md:px-5 pt-3 pb-1 flex items-start gap-3">
                    <div className="flex items-center justify-center w-[42px] h-[42px] md:w-[52px] md:h-[52px] rounded-xl bg-gradient-to-br from-[#8B0000] to-[#6B0000] text-white shadow-md shadow-red-900/20 shrink-0 mt-0.5">
                      <span className="text-[16px] md:text-[18px] font-black leading-none">{q.id}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <span className="text-[9px] font-bold text-[#8B0000] uppercase tracking-wider leading-none">Q.{q.id}</span>
                        <span className="w-[3px] h-[3px] rounded-full bg-gray-300 shrink-0" />
                        <span className="text-[10px] text-gray-400 leading-none">{q.examInfo || 'SSC Error Detection'}</span>
                      </div>
          ...
          <div className="mt-12 md:mt-14 mb-8 text-center">
            <button
              onClick={scrollToQuizSection}
              className="bg-[#8B0000] hover:bg-[#6B0000] active:bg-[#4A0000] text-white font-black text-[18px] md:text-[20px] px-10 md:px-16 py-4 md:py-5 rounded-xl shadow-lg hover:shadow-xl transition-all hover:scale-105 active:scale-95 inline-flex items-center gap-3"
            >
          ...
          <div className="flex items-center justify-center gap-4 mt-8 pb-8">
            {page > 1 ? (
              <Link
                href={`/error-detection/${page - 1}`}
                className="bg-white border border-amber-200/60 hover:bg-[#8B0000] hover:text-white hover:border-[#8B0000] text-gray-700 font-bold text-[14px] px-6 py-3 rounded-lg shadow-sm transition-all duration-200 active:scale-95"
              >
                &larr; Previous Page
              </Link>
            ) : (
            ...
            <div className="flex items-center gap-3 px-4 py-2 bg-white rounded-full border border-amber-100 shadow-sm">
        ...
        <div className="max-w-[860px] mx-auto px-5 w-full">
          <div className="relative">
            ...
            <div className="relative flex justify-center">
              <span className="bg-[#faf5ef] px-6 py-1 text-[12px] font-black text-gray-400 uppercase tracking-widest">Quiz Section</span>
            </div>
          </div>
        </div>
      )}
      ...
      {/* QUIZ SECTION */}
      <section ref={quizSectionRef} className={`bg-[#faf5ef] ${quizStarted ? "" : "pt-12 pb-16"}`}>
        {!quizStarted ? (
          <div className="max-w-[620px] mx-auto px-5 text-center">
            <div className="bg-white rounded-2xl border border-amber-100/50 shadow-sm p-10 md:p-12">
              <div className="w-20 h-20 bg-[#8B0000]/10 rounded-full flex items-center justify-center mx-auto mb-5">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                <button
                  onClick={handleStartQuiz}
                  className="bg-[#8B0000] hover:bg-[#6B0000] active:bg-[#4A0000] text-white font-black text-[18px] px-10 py-4 rounded-xl shadow-lg hover:shadow-xl transition-all hover:scale-105 active:scale-95 flex items-center gap-2"
                >
                  <span>&#9654;</span>
                  <span>Start Quiz</span>
                </button>
                <Link href="/?tab=error-detection" className="text-gray-400 hover:text-gray-600 text-[14px] font-bold transition-colors">
          ...
          <div className="border-t border-slate-200">
            <div className="max-w-[860px] mx-auto px-5 py-4">
              <details className="bg-white rounded-lg border border-gray-100 shadow-sm overflow-hidden">
                <summary className="bg-gradient-to-r from-[#0f172a] via-[#8B0000] to-[#0f172a] px-5 md:px-6 py-3.5 cursor-pointer hover:opacity-90 transition-opacity text-[14px] font-bold text-white flex items-center gap-2 select-none">
                  ...
                  <span>&#128214; Reading Passage &mdash; SSC Error Detection PYQs (Page {page})</span>
                </summary>
            ...
            <div className="flex flex-col bg-slate-50" style={{ minHeight: "calc(100vh - 96px)" }}>
              <div className="bg-slate-100 border-b border-slate-200 px-2 md:px-4 py-2 flex items-center overflow-x-auto whitespace-nowrap text-[10px] md:text-xs gap-1 shrink-0">
                <span className="text-[#8B0000] font-black uppercase mr-2 border-r border-slate-300 pr-2 tracking-wider text-[12px]">SSC Error Detection</span>
                <span className="text-slate-500 font-bold uppercase mr-2 text-[11px]">Page {page}</span>
                <button className="bg-[#8B0000] text-white px-4 py-1.5 rounded-t font-semibold text-xs">Quiz Mode</button>
              ...
              <main className="flex-1 flex flex-col lg:flex-row p-2 md:p-4 gap-3 md:gap-4 min-h-0">
                  ...
                  <div className="flex flex-col h-full min-h-0 max-w-full">
                    ...
                    <div className="shrink-0 px-5 pt-5 pb-4 border-b border-slate-100 bg-white rounded-t-lg">
                      <div className="flex items-center text-sm flex-wrap gap-y-2">
                          ...
                          <button
                            ...
                            className={`flex items-center gap-1 font-bold text-xs px-3 py-1.5 rounded-full shadow-sm transition-all duration-150 active:scale-95 ${
                              currentQIndex >= totalQ - 1
                                ? "bg-slate-100 text-slate-300 cursor-not-allowed shadow-none"
                                : "bg-[#8B0000] hover:bg-[#6B0000] text-white hover:shadow-md hover:scale-105"
                            }`}
                          >
                            <span>Next</span>
                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <span className="bg-[#8B0000] text-white font-semibold text-xs px-2.5 py-1 rounded-full">Q {currentQIndex + 1}/{totalQ}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto min-h-0 px-5 pb-5">
                      {currentQuestion.title && (
                        <div className="flex items-center gap-2 mt-4 mb-2">
                          <span className="bg-[#8B0000]/10 text-[#8B0000] text-[11px] font-bold px-2 py-0.5 rounded">Q.{currentQuestion.id}</span>
                      ...
                      <div className="space-y-[3px]">
                        {currentQuestion.options.map((opt, idx) => {
                          ...
                          let ringClass = "";
                          let radioColor = "text-[#8B0000] border-slate-300 focus:ring-[#8B0000]";
                      ...
                      <div className="flex items-center gap-2 mt-4 lg:hidden">
                        ...
                        <button
                          ...
                          className={`flex-1 flex items-center justify-center gap-1 font-bold text-xs px-3 py-2.5 rounded-lg shadow-sm transition-all duration-150 active:scale-95 ${
                            currentQIndex >= totalQ - 1
                              ? "bg-slate-100 text-slate-300 cursor-not-allowed shadow-none"
                              : "bg-[#8B0000] hover:bg-[#6B0000] text-white hover:shadow-md"
                          }`}
                        >
                          <span>Next</span>
                          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      {showExplanation && (
                        <div className="bg-amber-50 border border-amber-200 rounded-lg p-5 mt-5">
                          ...
                          {currentQuestion.fullExplanation && (<div className="reading-body text-[13px] md:text-[14px]">
                          <ReactMarkdown
                            remarkPlugins={[remarkGfm]}
                            components={markdownComponents}
                          >
                ...
                <div className="lg:hidden fixed bottom-5 right-5 z-50">
                    <button
                        ...
                        className="bg-[#8B0000] text-white w-12 h-12 rounded-full shadow-lg flex items-center justify-center hover:bg-[#6B0000] transition-all active:scale-90"
                    >
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {showMobileStats && (
                    <div className="lg:hidden fixed inset-0 z-50 flex justify-end">
                        ...
                        <div className="relative w-[280px] bg-white shadow-xl overflow-y-auto p-4 animate-in slide-in-from-right">
                            ...
                            <div className="bg-red-50 border border-red-100 rounded p-2 text-center mb-4">
                                <span className="block text-[9px] text-red-700 font-bold uppercase tracking-wider">Score</span>
                                <span className={`block text-xl font-black my-0.5 ${totalScore >= 0 ? "text-red-800" : "text-red-600"}`}>{totalScore.toFixed(2)}</span>
                                <span className="block text-[9px] text-red-600">{correctCount}/{totalQ} correct</span>
                            </div>
                            <div className="grid grid-cols-10 gap-[2px] justify-items-center mb-4">
                                {questions.map((q, idx) => {
                                    ...
                                    if (isActive) circleClass = "bg-[#8B0000] text-white ring-2 ring-red-300";
                            ...
                            <button onClick={() => { handleBackToReading(); setShowMobileStats(false); }}
                ...
                <div className="hidden lg:flex lg:w-[340px] flex-col gap-4 min-h-0">
                  <div className="bg-white border border-slate-200 rounded-lg p-4 shadow-sm shrink-0">
                    <div className="flex justify-between items-center mb-4 pb-2 border-b border-slate-100">
                      <div className="flex items-center gap-2">
                        <div className="w-7 h-7 bg-[#8B0000] text-white flex items-center justify-center font-bold text-xs rounded-full">ED</div>
                        <span className="font-bold text-xs text-slate-700">Error Detection</span>
                      </div>
                      <div className="relative">
                        <button
                              ...
                              {(["all", "correct", "incorrect", "skip"] as FilterMode[]).map((mode) => (
                                ...
                                >
                                  <span className={`w-2 h-2 rounded-full ${
                                    mode === "correct" ? "bg-emerald-500" : mode === "incorrect" ? "bg-rose-500" : mode === "skip" ? "bg-amber-400" : "bg-red-600"
                                  }`} />
                    ...
                    <div className="bg-red-50 border border-red-100 rounded p-2 text-center">
                      <span className="block text-[9px] text-red-700 font-bold uppercase tracking-wider">Total Score</span>
                      <span className={`block text-xl font-black my-0.5 ${totalScore >= 0 ? "text-red-800" : "text-red-600"}`}>
                  ...
                  <div className="bg-white border border-slate-200 rounded-lg p-3 shadow-sm flex-1 min-h-0 flex flex-col">
                    <div ref={paletteRef} className="flex-1 overflow-y-auto min-h-0">
                      <div className="grid grid-cols-10 gap-[1px] justify-items-center">
                        {questions.map((q, idx) => {
                          ...
                          if (isActive) {
                            circleClass = "bg-[#8B0000] text-white ring-[1.5px] ring-red-300";
                    ...
                    <div className="grid grid-cols-1 gap-2 mt-3 pt-3 border-t border-slate-100 shrink-0">
                      <div className="flex gap-2">
                        {page > 1 && (
                          <Link href={`/error-detection/${page - 1}`} className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 text-[10px] font-bold py-2 rounded text-center transition">
                            &larr; Prev Page
                          </Link>
                        )}
                        {page < totalPages && (
                          <Link href={`/error-detection/${page + 1}`} className="flex-1 bg-[#8B0000]/10 hover:bg-[#8B0000]/20 text-[#8B0000] text-[10px] font-bold py-2 rounded text-center transition">
                            Next Page &rarr;
                          </Link>
                        )}
                      </div>
